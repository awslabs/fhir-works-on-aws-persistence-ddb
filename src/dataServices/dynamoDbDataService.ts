    ...
    async createResource(request: CreateResourceRequest) {
        ...
        try {
            ...
            if (e.code === 'ConditionalCheckFailedException') {
                // It is highly unlikely that the autogenerated id will collide with a preexisting id, but adding that case
                // to cover all bases
                const errorMessage = id
                    ? `Unable to create resource with id ${id}, because resource already exists`
                    : 'Auto generated id matched an existing id, please try again';
                throw new InvalidResourceError(errorMessage);
            }
            throw e;
        }
